...
<div class="md:relative absolute top-10 left-[50%] md:top-0 md:left-0 -translate-x-1/2 md:translate-0">
	<Button classname={clsx("bg-white shadow py-2 px-4 hover:bg-gray-200",
		showDD ? "rounded-t-3xl rounded-b-none" : "")}
        on:click={() => showDD = !showDD}>
		AWS | {$datastore.selectedRegion}
	</Button>
    {#if showDD}
    <div class="absolute w-full bg-white shadow z-30 text-sm max-h-60 overflow-auto rounded-b-3xl">
        {#each Object.keys(awsRegions) as region, index (index)}
            ...
            {#each awsRegions[region] as item, idx (idx)}
                <button class="p-2 hover:bg-gray-100 w-full text-start" on:click={(e) => {
                	e.stopPropagation();
